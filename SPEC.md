...
- **Overlay window isolated onto its own thread.** Real-device testing (Tecno Spark 20C / HiOS) found `WindowManager.addView()` in `FloatingWindowOverlayService` can hang indefinitely — a Binder call to the OEM's WindowManagerService that never returns, confirmed via the process sitting in kernel state `D` (uninterruptible sleep) with 0% CPU. Because that Service shares a process — and therefore a main thread — with `CallMonitorService`, this hang froze the *entire app*, including call detection, after the first overlay attempt: exactly matching a report of "the popup only shows right after opening the app" (each reopen effectively got one attempt in before the process wedged again). Fixed by doing all `WindowManager`/`ComposeView` work in `FloatingWindowOverlayService` on a dedicated `HandlerThread` instead of main, and by moving `CallMonitorService`'s `TelephonyCallback` registration and `CallLog` `ContentObserver` dispatch onto their own `HandlerThread` too, so call detection can no longer be taken down by a hang anywhere else in the app.
